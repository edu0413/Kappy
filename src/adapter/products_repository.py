...
#GENERATED BY DEFAULT AS IDENTITY
    ...
    def get_all_ids(self): #Will get all Event ID's
        with self.con.cursor() as cursor:
    ...
    def get_category_ids(self, category): #Will get all Event ID's by categories
        with self.con.cursor() as cursor:
            cursor.execute("SELECT product_id FROM products WHERE category=%s;", (category,)) 
            result = cursor.fetchall()
            if len(result) != 0:
                return result
            else:
    ...
    def publish_product(self, image, description, title, category, price, discount, discounted_price, stock, vendor, on_request, vendor_email, vendor_phone, meta_title, meta_description, meta_tags, slug): #Will insert the requested Event data into the Products Table, and will return the product_id
        with self.con.cursor() as cursor:
            cursor.execute("INSERT INTO products(image, description, title, category, price, discount, discounted_price, stock, vendor, on_request, vendor_email, vendor_phone, meta_title, meta_description, meta_tags, slug) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s) RETURNING product_id;",
                           (image, description, title, category, price, discount, discounted_price, stock, vendor, on_request, vendor_email, vendor_phone, meta_title, meta_description, meta_tags, slug))
    ...
    def list_products(self):
        with self.con.cursor() as cursor:
            cursor.execute("SELECT product_id, title, category, price, on_request, created_at FROM products;")
            result = cursor.fetchall()
            if len(result) != 0:
                return result
            else:
